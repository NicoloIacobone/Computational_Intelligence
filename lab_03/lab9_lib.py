...
class AbstractProblem:
    ...
    # def __call__(self, genome):
    #     self._calls += 1
    #     fitnesses = sorted((self.print_and_return(genome, s) for s in range(self.x)), reverse=True)
    #     print("fitnesses", fitnesses)
    #     val = sum(f for f in fitnesses if f == fitnesses[0]) - sum(
    #         f * (0.1 ** (k + 1)) for k, f in enumerate(f for f in fitnesses if f < fitnesses[0])
    #     )
    ...
    # def print_and_return(self, genome, s):
    #     subsequence = genome[s :: self.x]
    #     print("Subsequence for s =", s, "is", subsequence)
    #     return AbstractProblem.onemax(subsequence)
